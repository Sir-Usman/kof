                ...
                <div class="cz-sidebar  box-shadow-lg" id="shop-sidebar" style="margin-bottom: -10px;border-radius: 5px;">
                    ...
                    <div class="pb-0" >
                        ...
                        <div class="text-center">
                            <div class="" style="border-bottom: 1px solid #F3F5F9; padding:10px;">
                            ...
                            <div class="form-inline flex-nowrap {{Session::get('direction') === "rtl" ? 'ml-sm-4' : 'mr-sm-4'}} pb-3 for-mobile"
                                style="width: 100%; padding: 14px; padding-top: 0px;">
                    ...
                    <div class="px-3">
                        <div class="">
                            <input type="checkbox" @if(request()->freeDelivery == 'true') checked @endif data-toggle="toggle" data-size="lg" id="filterFreeDeliveryLeft"
                            {{-- data-url="{{route('products',['id'=> $data['id'],'data_from'=>'search','page'=>1])}}" --}}
                            data-url="{{route('products',['id'=> $data['id'],'data_from'=>'search','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}"
                            >
                            <label class="pointer" for="filterFreeDeliveryLeft">Free Delivery </label>
                        </div>
                        <div class="">
                            <input type="checkbox" @if(request()->returnAccpeted == 'true') checked @endif data-toggle="toggle" data-size="lg" id="filterReturnAcceptedLeft"
                            {{-- data-url="{{route('products',['id'=> $data['id'],'data_from'=>'search','page'=>1])}}" --}}
                            data-url="{{route('products',['id'=> $data['id'],'data_from'=>'search','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}">
                            <label class="pointer" for="filterReturnAcceptedLeft">Return Accepted </label>
                        </div>
                        <div class="">
                            <input type="checkbox" @if(request()->saalsiPayment == 'true') checked @endif data-toggle="toggle" data-size="lg" id="filterSaalsiPaymentLeft"
                            {{-- data-url="{{route('products',['id'=> $data['id'],'data_from'=>'search','page'=>1])}}" --}}
                            data-url="{{route('products',['id'=> $data['id'],'data_from'=>'search','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}">
                            <label class="pointer" for="filterSaalsiPaymentLeft">Saalsi Payment </label>
                        </div>
                        <div class="">
                            <input type="checkbox" @if(request()->cod == 'true') checked @endif data-toggle="toggle" data-size="lg" value="" id="filterCODLeft"
                            {{-- data-url="{{route('products',['id'=> $data['id'],'data_from'=>'search','page'=>1])}}" --}}
                            data-url="{{route('products',['id'=> $data['id'],'data_from'=>'search','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}">
                            <label class="pointer" for="filterCODLeft">Cash On Delivery </label>
                        </div>
                    </div>
                    ...
                    <div class="px-3">
                        <div class="py-2">
                            <span class="widget-title" style="font-weight: 600;">{{\App\CPU\translate('Location')}} </span>
                        </div>
                        <?php
                            $cities = \Cache::rememberForever('cities', function() {
                                return \App\Model\City::get();
                            });
                        ?>
                        <select class="form-control" name="city_id" id="cty_idLeft">
                            <option value="" data-url="{{route('products',['id'=> $data['id'],'data_from'=>'search','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}">All</option>
                            @foreach($cities as $city)
                                <option value="{{ $city->id }}" data-url="{{route('products',['city_id' => $city->id, 'id'=> $data['id'],'data_from'=>'search','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}" @if(request()->city_id == $city->id) selected @endif>{{ $city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="px-3">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <div style="width: 20%;background:#1B7FED;width:30px;height:35px;border-radius:3px;" class="d-flex pointer justify-content-center align-items-center" onclick="searchByPrice()">
                                <a class="">
                            ...
                            @foreach(\App\CPU\BrandManager::get_active_brands() as $brand)
                                <div class="brand mt-2 for-brand-hover {{Session::get('direction') === "rtl" ? 'mr-2' : ''}}" id="brand">
                                    <li style="cursor: pointer;padding: 2px;padding-right:15px;" class="flex-between"
                                        onclick="location.href='{{route('products',['id'=> $brand['id'],'data_from'=>'brand','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}'">
                                        <div>
                                            {{ $brand['name'] }}
                                        </div>
                                        @if($brand['brand_products_count'] > 0 )
                                            <div style="background: #F3F5F9; border-radius: 10px;padding: 2px 7px 2px;color:#212629;font-weight: 400;font-size: 12px;">
                                                <span class="">
                            ...
                            @foreach($categories as $category)
                                <div >
                                    <div class="card-header p-1 flex-between">
                                        <div>
                                            <label class="for-hover-lable" style="cursor: pointer" onclick="location.href='{{ route('products',['id'=> $category['id'],'data_from'=>'category','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}'">
                                                {{$category['name']}}
                                            </label>
                                        </div>
                                        <div>
                                            ...
                                            <div class=" for-hover-lable card-header p-1 flex-between">
                                                <div>
                                                    <label style="cursor: pointer"
                                                            onclick="location.href='{{ route('products',['id'=> $child['id'],'data_from'=>'category','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}'">
                                                        {{$child['name']}}
                                                    </label>
                                                </div>
                                                <div>
                                                ...
                                                @foreach($child->childes as $ch)
                                                    <div class="card-header p-1">
                                                        <label class="for-hover-lable" style="cursor: pointer"
                                                                onclick="location.href='{{ route('products',['id'=> $ch['id'],'data_from'=>'category','page'=>1]).http_build_query(request()->query(), null, '&', PHP_QUERY_RFC3986) }}'">{{$ch['name']}}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @endforeach
                ...
                <aside class="" style="padding-right: 5%;padding-left: 5%;">
                    <div class="" id="shop-sidebar" style="margin-bottom: -10px;">
                        ...
                        <div class="" style="padding-top: 12px;">
                             <!--Filter -->
                            <div class="widget cz-filter" style="width: 100%; margin-top:30px">
                    ...
                    <div class="" id="shop-sidebar" style="margin-bottom: 11px;">
                        ...
                        <div class="">
                            ...
                            <div class="widget cz-filter mb-4 pb-4- border-bottom mt-2-">
                                ...
                                <div class="input-group-overlay input-group-sm mb-2">
                                    <input style="background: aliceblue" class="cz-filter-search form-control form-control-sm appended-form-control"
...
@push('script')
    <script>
        ...
        function searchByPrice() {
            ...
            let freeDelivery = $('#freeDeliveryLeft').val();
            let returnAccpeted = $('#returnAccpetedLeft').val();
            let cod = $('#codLeft').val();
            let saalsiPayment = $('#saalsiPaymentLeft').val();
            let city_id = $('#cty_idLeft').val();
            $.get({
                url: '{{url('/')}}/products',
                data: {
                    id: '{{$data['id']}}',
                    ...
                    returnAccpeted: returnAccpeted,
                    cod: cod,
                    saalsiPayment: saalsiPayment,
                    city_id: city_id,
                    min_price: min,
                    max_price: max,
                },
        ...
        $('#cty_idLeft').change(function () {
            var href = $(this).find('option:selected').attr("data-url");
            if (href) {
        ...
        $('#filterFreeDeliveryLeft').change(function() {
            href = $(this).attr("data-url");
            if($(this).is(":checked")) {
                window.location = href + "&freeDelivery=true";
            }else{
                window.location = href + "&freeDelivery=false";
            }
        });

        $('#filterReturnAcceptedLeft').change(function() {
            href = $(this).attr("data-url");
            if($(this).is(":checked")) {
                window.location = href + "&returnAccpeted=true";
            }else{
                window.location = href + "&returnAccpeted=false";
            }
        });

        $('#filterSaalsiPaymentLeft').change(function() {
            href = $(this).attr("data-url");
            if($(this).is(":checked")) {
                window.location = href + "&saalsiPayment=true";
            }else{
                window.location = href + "&saalsiPayment=false";
            }
        });

        $('#filterCODLeft').change(function() {
            href = $(this).attr("data-url");
            if($(this).is(":checked")) {
                window.location = href + "&cod=true";
            }else{
                window.location = href + "&cod=false";
            }
        });
