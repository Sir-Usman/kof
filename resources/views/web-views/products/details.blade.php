...
@push('css_or_js')
    ...
    <style>
        ...
        .slider-icon .active,
        .slider-icon .demo:hover {
            opacity: 1;
        }
        
        .slider-icon img.hover-shadow {
            transition: 0.3s;
        }
    </style>
@endpush
...
@section('content')
                            ...
                            <div class="cz-preview">
                                ...
                                @if($product->images!=null)
                                    @foreach ($imgs as $key => $photo)
                                        <div href="{{asset("storage/app/public/product/$photo")}}"
                                            class="cz-preview-item d-flex align-items-center justify-content-center {{$key==0?'active':''}}"
                                            id="image{{$key}}"
                                            onclick="openModal();currentSlide({{ $key+1 }})" class="pointer"
                                            >
                            ...
                            @if(!$product['external_link'])
                                ...
                                @else
                                    <a href="{{ $product['external_link'] }}"
                                        ...
                                        onclick="buy_now()"
                                        type="button"
                                        style="margin-top: 100px; width:50%; height: 45px; background: #FFA825 !important; color: #ffffff;">
                                        <span class="string-limit">{{ $product['external_link_btn'] }}</span>
                                    </a>
                                @endif
                            </form>
    ...
    {{-- image slider modal --}}
    <div id="myModal" class="slider-modal" style="z-index:9999">
        <span class="close-btn pointer" onclick="closeModal()"><i class="fa fa-close"></i></span>
        <div class="slider-modal-content">
            @foreach ($imgs as $key => $photo)
                <div class="mySlides">
                    <div class="numbertext">{{ $key + 1 }} / {{ $count_imgs }}</div>
            ...
            <div style="position:fixed; bottom:0;">
                @foreach ($imgs as $key => $photo)
                    <div class="slider-icon">
                        <img class="demo cursor" src="{{asset("storage/app/public/product/$photo")}}" style="width:100%" onclick="currentSlide({{ $key+1 }})">
                    </div>
                @endforeach
            </div>
        </div>
