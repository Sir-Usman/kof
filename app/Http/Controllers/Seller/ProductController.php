...
class ProductController extends Controller
{
    ...
    public function add_new1()
    {
        $cat = Category::where(['parent_id' => 0])->get();
        $br = Brand::orderBY('name', 'ASC')->get();
        $brand_setting = BusinessSetting::where('type', 'product_brand')->first()->value;
        $digital_product_setting = BusinessSetting::where('type', 'digital_product')->first()->value;
        return view('seller-views.product.add', compact('cat', 'br', 'brand_setting', 'digital_product_setting'));
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'image'                 => 'required',
            'tax'                   => 'required|min:0',
            'unit_price'           => 'required_without:external_link|nullable|numeric|min:1', // required_without + nullable added by hassan005004 + as per crypto adding 
        ...
        if ($request['unit_price'] <= $dis && !$request->external_link) {
            $validator->after(function ($validator) {
                $validator->errors()->add(
    ...
    function affiliate(){
        return view('seller-views.product.affiliate');
    }
    function add(){
        return view ('seller-views.product.add');
    }
    function add1(Request $request){
        ...
        $affiliate->buy = $request->buy;
        // $image_name = $image->getClientOriginalName();
        // $affiliate->storeAs('public/images', $image_name);
        // $affiliate->image = $image_name;
        // $affiliates->save();
        return redirect ('seller-views.product.affiliate');
    }
    ...
    function list1(Request $request)
    {
        ...
        return view('seller-views.product.affiliate', compact('products', 'search'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'unit'                  => 'required_if:product_type,==,physical',
            'tax'                   => 'required|min:0',
            'unit_price'            => 'required_without:external_link|nullable|numeric|min:0', // hassan005004 + as per crypto adding 
            // 'unit_price'            => 'required|numeric|min:1',
            'purchase_price'        => 'required_without:external_link|nullable|numeric|min:0', // hassan005004 + as per crypto adding 
        ...
        // added by hassan005004
        $product->external_link        = $request->external_link ? $request->external_link : null;
        $product->external_link_btn    = $request->external_link_btn ? $request->external_link_btn : null;
