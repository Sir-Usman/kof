...
class CustomerController extends Controller
{
    ...
    public function add_new_address(Request $request)
    {
        ...
        $country = $request->country ? $request->country : 'Pakistan';
        $zip = $request->zip ? $request->zip : 60000;

        $zip_restrict_status = Helpers::get_business_settings('delivery_zip_code_area_restriction');
        $country_restrict_status = Helpers::get_business_settings('delivery_country_restriction');

        if ($country_restrict_status && !self::delivery_country_exist_check($country)) {
            return response()->json(['message' => translate('Delivery_unavailable_for_this_country')], 403);

        } elseif ($zip_restrict_status && !self::delivery_zipcode_exist_check($zip)) {
