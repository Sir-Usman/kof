...
class WebController extends Controller
{
    ...
    public function all_brands(Request $request)
    {
        ...
        // if($search != ""){
        $brands = Brand::whereIf('name', 'Like', '%' . $request->brand_name . '%')->paginate(24);
        // }else{
        // $brands = Brand::all();
        // }
    ...
    public function searched_products(Request $request)
    {
        ...
        $result = ProductManager::search_products_with_filter($request['name'], $limit = 10, $offset = 0, $request['minPrice'], $request['maxPrice'], $request['freeDilevery'], $request['returnAccepted'], $request['cod'], $request['saalsiPayment']);
    ...
    public function products(Request $request)
    {
        ...
        // search filters added by hassan005004 on seller table
        if ($request->saalsiPayment == 'true' || $request->city_id) {
            $porduct_data = $porduct_data->whereHas('seller', function ($q) use ($request) {
                $q->whereIf('accept_saalsi_payment', '=', (bool)$request->saalsiPayment)
                    ->whereIf('city_id', '=', $request->city_id);
            });
        }

        // search filters added by hassan005004 on product table
        if ($request->freeDelivery == 'true') {
            // $porduct_data = $porduct_data->where('free_shipping', 1);
            $porduct_data = $porduct_data->where('shipping_cost', 0);
        }
        $porduct_data = $porduct_data->whereIf('refundable', '=', (bool)$request->returnAccpeted);
    ...
    public function privacy_policy_2()
    {
        $privacy_policy = BusinessSetting::where('type', 'privacy_policy_2')->first();
        return view('web-views.privacy-policy-2', compact('privacy_policy'));
    }
    ...
    public function updateFcmToken(Request $request)
    {
        if (auth('customer')->check()) {
            try {
                if (auth('customer')->user()->fcm_web_token != $request->token) {
                    $user = User::find(auth('customer')->user()->id);
                    $user->fcm_web_token = $request->token;
                    $user->save();
                    // User::where()->first()->update(['fcm_web_token' => $request->token]);
                    return response()->json([
                        'success' => true
                    ]);
                }
    ...
    public function notification(Request $request)
    {
        ...
        try {
            $fcmTokens = Seller::whereNotNull('fcm_web_token')->pluck('fcm_web_token')->toArray();
            $arr = [
                'title' => 'Message - This is test message',
                'body' => 'This is test message',
            ];
            $data = [
                'type' => 'info'
            ];
            return General::sendWebNotification($arr, $data, $fcmTokens);

            $fcmTokens = User::whereNotNull('fcm_web_token')->pluck('fcm_web_token')->toArray();
            $arr = [
                'title' => 'Message - This is test message',
                'body' => 'This is test message',
            ];
    ...
    {
        // return 123;
        $categories = DirectsellCategories::with('subcategories')->get();
        return view('web-views.sell-now', compact('categories'));
    }

    public function product_details($category_id)
    {
        $category = DirectsellCategories::find($category_id);

        if ($category) {
            $subcategories = $category->subcategories;
            return view('web-views.product-details')->with('subcategories', $subcategories);
        } else {
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Fetch the actual subcategory names based on the provided IDs
            $subcategories = DirectsellSubCategories::whereIn('id', $validatedData['sub_categories'])->pluck('name')->toArray();

            // Convert subcategories array to a comma-separated string
            ...
            $product = Product_detail::create([
                ...
                'sub_categories' => $subcategoriesString,
