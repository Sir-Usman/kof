...
class SellerController extends Controller
{
    public function index(Request $request)
    {
        ...
        $sellers = Seller::with(['orders', 'product', 'shops'])
    ...
    public function updateSellerRecord(Request $request)
    {
        try {
            ...
            if (isset($request->photo)) {
                $seller->update([
                    "image"      => ImageManager::upload('seller/', 'png', $request->file('photo'))
                ]);
            }
            Shop::where('seller_id', $request->id)->update([
                'name' => $request->shopName,
                'address' => $request->shopAddress,
                'contact' => $request->shopContact
            ]);

            try {
                Mail::to('support@khareedofarokht.pk')->send(new RequestMail($seller));
