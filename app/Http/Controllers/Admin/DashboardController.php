...
class DashboardController extends Controller
{
    ...
    public function order_stats(Request $request)
    {
        ...
        if($request->start_date || $request->end_date){
            session()->put('datewise', 'yes');
            session()->put('dd_ff_start_date', $request->start_date);
            session()->put('dd_ff_end_date', $request->end_date);
        }else{
            session()->put('datewise', 'no');
        }
    ...
    public function common_query_order_stats($query){
        ...
        $data = [];
        $is_datewise = session()->has('datewise') && session('datewise') == 'yes' ? 1 : 0;
        if($is_datewise){
            $today = 0;
            $this_month = 0;
            $data['start_date'] = session()->get('dd_ff_start_date');
            $data['end_date'] = session()->get('dd_ff_end_date');
        }
        ...
        return $query->when($today, function ($query) {
            ...
            ->when($is_datewise, function ($query) use($data) {
                $d_s = Carbon::parse($data['start_date'])->format('Y-m-d H:i:s');
                $d_e = Carbon::parse($data['end_date'])->format('Y-m-d H:i:s');
                return $query->whereBetween('created_at', [$d_s, $d_e]);
            })
            ->count();
    }
