        ...
        try {
            ...
            if ($socialLoginServices) {
                foreach ($socialLoginServices as $socialLoginService) {
                    ...
                    elseif ($socialLoginService['status'] == true && $socialLoginService['login_medium'] == 'apple') {
                        $apple_config = array(
                            'client_id' => $socialLoginService['client_id'],
                            'client_secret' => $socialLoginService['client_secret'],
                            'redirect' => url('customer/auth/login/apple/callback'),
                        );
                        Config::set('services.apple', $apple_config);
                    }
                }
            }
