...
class ProductManager
{
    ...
    public static function search_products_with_filter($name, $limit = 10, $offset = 1,$min_price,$max_price,$freeDelivery,$returnAccepted,$cod,$saalsi)
    {
       
        /*$key = explode(' ', $name);*/
        $key = explode(' ', $name);
        $freeShipping = $freeDelivery == "true" ? "1" : "0";
        $refundable = $returnAccepted == "true" ? "1" : "0";
        $minPrice =$min_price != "" ? $min_price : 0;
        $maxPrice = $max_price != "" ? $max_price : 100000000;
        ...
        })->where('free_shipping',$freeShipping)
        ->where('refundable',$refundable)
        ->where('unit_price','>',$minPrice)->where('unit_price','<',$maxPrice)
                ->paginate($limit, ['*'], 'page', $offset);
