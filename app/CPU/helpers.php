...
class Helpers
{
    ...
    public static function get_shipping_methods($seller_id, $type)
    {
        if ($type == 'admin') {
            ...
            // here check if seller enable shipping method then show else show for admin
            $seller = Seller::where('id', $seller_id)->first();
            if($seller->is_self_shipping == 1){
                $shipping_methods = ShippingMethod::where(['status' => 1])->where(['creator_id' => $seller_id, 'creator_type' => $type])->get();
